...
def build_winner(summaries: list[dict]) -> dict:
    ...
    return {
        ...
        "justificationHe": (
            f"השיטה המהירה ביותר מבין אלה שמכסות את כל הנוסחאות בקטלוג: {winner['totalRuntimeMs']:.0f} ms בסך הכול, "
            f"{winner['rowsPerSec']:.0f} שורות לשנייה — פי {winner['speedupVsSlowest']:.0f} מהירה יותר מהשיטה האיטית "
            "ביותר. כל נוסחה מהודרת פעם אחת ל-delegate שנשמר במטמון, כך שאין צורך לפענח את הנוסחה מחדש בכל שורה, "
            "ואין פנייה נוספת למסד הנתונים עבור כל נוסחה."
        ),
    }
